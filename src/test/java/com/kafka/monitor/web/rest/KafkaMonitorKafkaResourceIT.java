...
import com.kafka.monitor.config.KafkaProperties;
...
class KafkaMonitorKafkaResourceIT {
    ...
    void setup() {
        ...
        KafkaMonitorKafkaResource kafkaResource = new KafkaMonitorKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/kafka-monitor-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/kafka-monitor-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
